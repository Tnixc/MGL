...
+ (id)rendererWithSDLWindow:(SDL_Window *)window
{
    GLMContext ctx = createGLMContext(GL_RGBA, GL_UNSIGNED_BYTE, GL_DEPTH_COMPONENT24, GL_UNSIGNED_INT,
...
- (bool)mapGLBuffersToMTLBufferMap:(BufferMapList *)buffer_map stage:(int)stage
{
    ...
    struct
    {
        ...
        const char *name;
                         ...
                         {SPVC_RESOURCE_TYPE_PUSH_CONSTANT, _UNIFORM_CONSTANT, "Uniform Constant"},
...
- (bool)bindVertexBuffersToCurrentRenderEncoder
{
    ...
    for (int i = 0; i < ctx->state.vertex_buffer_map_list.count; i++)
    {
        ...
        if (ptr->size < 4096)
        {
            ...
            // Print first few floats from vertex data
...
- (int)getProgramBinding:(int)stage type:(int)type index:(int)index
{
    ...
    if (index >= ptr->spirv_resources_list[stage][type].count)
        return -1;
...
- (id<MTLLibrary>)compileShader:(const char *)str
{
    ...
    if (!library)
    {
        ...
        fprintf(stderr, "Metal shader source:\n%s\n", str);
    }
    assert(library);
...
- (bool)bindMTLProgram:(Program *)ptr
{
    ...
    if (ptr->dirty_bits & DIRTY_PROGRAM)
    {
        ...
        for (int i = _VERTEX_SHADER; i < _MAX_SHADER_TYPES; i++)
        {
            if (ptr->mtl_data[i].library)
            {
                ...
                ptr->mtl_data[i].function = NULL;
                // Note: entry_point persists and doesn't need to be freed here
                // It will be freed when the program is deleted
            }
        }
    ...
    for (int i = _VERTEX_SHADER; i < _MAX_SHADER_TYPES; i++)
    {
        // Check if we have SPIRV/MSL for this stage (regardless of whether shader is still attached)
        if (ptr->spirv[i].msl_str)
        {
            if (ptr->mtl_data[i].library == NULL)
            {
                id<MTLLibrary> library;
                ...
                assert(function);
                ptr->mtl_data[i].library = (void *)CFBridgingRetain(library);
                ptr->mtl_data[i].function = (void *)CFBridgingRetain(function);
            }
        }
    }
...
- (MTLRenderPipelineDescriptor *)generatePipelineDescriptor
{
    ...
    vertexFunction = (__bridge id<MTLFunction>)(program->mtl_data[_VERTEX_SHADER].function);
    fragmentFunction = (__bridge id<MTLFunction>)(program->mtl_data[_FRAGMENT_SHADER].function);
...
- (bool)bindBuffersToComputeEncoder:(id<MTLComputeCommandEncoder>)computeCommandEncoder
{
    ...
    // Check for dirty buffer data and update if needed (like fragment/vertex shaders do)
    if ([self checkForDirtyBufferData:&ctx->state.compute_buffer_map_list])
    {
        RETURN_FALSE_ON_FAILURE([self updateDirtyBaseBufferList:&ctx->state.compute_buffer_map_list]);
    }
    ...
    {
        map = &ctx->state.compute_buffer_map_list.buffers[i];
        ...
        if (ptr->size < 4096)
        {
            ...
            // Debug: print the first float value if it's a float buffer
            if (ptr->size == sizeof(float))
            {
                float value = *(float *)ptr->data.buffer_data;
                static int debug_count = 0;
                if (debug_count++ % 60 == 0)
                {
                    NSLog(@"Setting compute buffer %d with float value: %f", i, value);
                }
            }

            [computeCommandEncoder setBytes:(const void *)ptr->data.buffer_data length:ptr->size atIndex:i];
        ...
        else
        {
            ...
            [computeCommandEncoder setBuffer:buffer offset:offset atIndex:i];
        }
    }
...
- (bool)processCompute:(id<MTLComputeCommandEncoder>)computeCommandEncoder
{
    ...
    // Get Metal function from program's mtl_data, not from shader_slots
    assert(program->mtl_data[_COMPUTE_SHADER].function);

    id<MTLFunction> func;
    func = (__bridge id<MTLFunction>)(program->mtl_data[_COMPUTE_SHADER].function);
    assert(func);
                   ...
                   groupsZ:(GLuint)groups_z
{
    ...
    {
        // groups_x/y/z from glDispatchCompute already represent the number of workgroups
        // We should NOT divide by local_workgroup_size - that's already handled by Metal
                ...
                indices:(const void *)indices
{
    ...
    {
        DEBUG_PRINT("mtlDrawElements: No element buffer bound, skipping draw\n");
        return;
    }
