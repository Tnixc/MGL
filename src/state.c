...
#define ENABLE_CAP(_cap_)                                                                                              \
    ctx->state.caps._cap_ = true;                                                                                      \
    break
#define DISABLE_CAP(_cap_)                                                                                             \
    ctx->state.caps._cap_ = false;                                                                                     \
    break
...
void mglDisable(GLMContext ctx, GLenum cap)
{
    ...
    case GL_BLEND:
        DISABLE_CAP(blend);
    case GL_LINE_SMOOTH:
        DISABLE_CAP(line_smooth);
    case GL_POLYGON_SMOOTH:
        DISABLE_CAP(polygon_smooth);
    case GL_CULL_FACE:
        DISABLE_CAP(cull_face);
    case GL_DEPTH_TEST:
        DISABLE_CAP(depth_test);
    case GL_STENCIL_TEST:
        DISABLE_CAP(stencil_test);
    case GL_DITHER:
        DISABLE_CAP(dither);
    case GL_SCISSOR_TEST:
        DISABLE_CAP(scissor_test);
    case GL_COLOR_LOGIC_OP:
        DISABLE_CAP(color_logic_op);
    case GL_POLYGON_OFFSET_POINT:
        DISABLE_CAP(polygon_offset_point);
    case GL_POLYGON_OFFSET_LINE:
        DISABLE_CAP(polygon_offset_line);
    case GL_POLYGON_OFFSET_FILL:
        DISABLE_CAP(polygon_offset_fill);
    case GL_MULTISAMPLE:
        DISABLE_CAP(multisample);
    case GL_SAMPLE_ALPHA_TO_COVERAGE:
        DISABLE_CAP(sample_alpha_to_coverage);
    case GL_SAMPLE_ALPHA_TO_ONE:
        DISABLE_CAP(sample_alpha_to_one);
    case GL_SAMPLE_COVERAGE:
        DISABLE_CAP(sample_coverage);
    case GL_RASTERIZER_DISCARD:
        DISABLE_CAP(rasterizer_discard);
    case GL_FRAMEBUFFER_SRGB:
        DISABLE_CAP(framebuffer_srgb);
    // case GL_PRIMITIVE_RESTART: DISABLE_CAP(primitive_restart);
    case GL_DEPTH_CLAMP:
        DISABLE_CAP(depth_clamp);
    case GL_TEXTURE_CUBE_MAP_SEAMLESS:
        DISABLE_CAP(texture_cube_map_seamless);
    case GL_SAMPLE_MASK:
        DISABLE_CAP(sample_mask);
    case GL_SAMPLE_SHADING:
        DISABLE_CAP(sample_shading);
    // case GL_PRIMITIVE_RESTART_FIXED_INDEX: DISABLE_CAP(primitive_restart_fixed_index);
    case GL_DEBUG_OUTPUT_SYNCHRONOUS:
        DISABLE_CAP(debug_output_synchronous);
    case GL_DEBUG_OUTPUT:
        DISABLE_CAP(debug_output);
    default:
...
void mglEnable(GLMContext ctx, GLenum cap)
{
    ...
    case GL_BLEND:
        ENABLE_CAP(blend);
    case GL_LINE_SMOOTH:
        ENABLE_CAP(line_smooth);
    case GL_POLYGON_SMOOTH:
        ENABLE_CAP(polygon_smooth);
    case GL_CULL_FACE:
        ENABLE_CAP(cull_face);
    case GL_DEPTH_TEST:
        ENABLE_CAP(depth_test);
    case GL_STENCIL_TEST:
        ENABLE_CAP(stencil_test);
    case GL_DITHER:
        ENABLE_CAP(dither);
    case GL_SCISSOR_TEST:
        ENABLE_CAP(scissor_test);
    case GL_COLOR_LOGIC_OP:
        ENABLE_CAP(color_logic_op);
    case GL_POLYGON_OFFSET_POINT:
        ENABLE_CAP(polygon_offset_point);
    case GL_POLYGON_OFFSET_LINE:
        ENABLE_CAP(polygon_offset_line);
    case GL_POLYGON_OFFSET_FILL:
        ENABLE_CAP(polygon_offset_fill);
    case GL_PROGRAM_POINT_SIZE:
        ENABLE_CAP(program_point_size);
    case GL_MULTISAMPLE:
        ENABLE_CAP(multisample);
    case GL_SAMPLE_ALPHA_TO_COVERAGE:
        ENABLE_CAP(sample_alpha_to_coverage);
    case GL_SAMPLE_ALPHA_TO_ONE:
        ENABLE_CAP(sample_alpha_to_one);
    case GL_SAMPLE_COVERAGE:
        ENABLE_CAP(sample_coverage);
    case GL_RASTERIZER_DISCARD:
        ENABLE_CAP(rasterizer_discard);
    case GL_FRAMEBUFFER_SRGB:
        ENABLE_CAP(framebuffer_srgb);
        // case GL_PRIMITIVE_RESTART: ENABLE_CAP(primitive_restart);
    case GL_DEPTH_CLAMP:
        ENABLE_CAP(depth_clamp);
    case GL_TEXTURE_CUBE_MAP_SEAMLESS:
        ENABLE_CAP(texture_cube_map_seamless);
    case GL_SAMPLE_MASK:
        ENABLE_CAP(sample_mask);
    case GL_SAMPLE_SHADING:
        ENABLE_CAP(sample_shading);
    // case GL_PRIMITIVE_RESTART_FIXED_INDEX: ENABLE_CAP(primitive_restart_fixed_index);
    case GL_DEBUG_OUTPUT_SYNCHRONOUS:
        ENABLE_CAP(debug_output_synchronous);
    case GL_DEBUG_OUTPUT:
        ENABLE_CAP(debug_output);
    default:
        ERROR_RETURN(GL_INVALID_ENUM);
        break;
    }
...
#define HINT(_target_)                                                                                                 \
    ctx->state.hints._target_ = mode;                                                                                  \
    break;
void mglHint(GLMContext ctx, GLenum target, GLenum mode)
{
    ...
    case GL_LINE_SMOOTH_HINT:
        HINT(line_smooth_hint);
    case GL_POLYGON_SMOOTH_HINT:
        HINT(polygon_smooth_hint)
    case GL_TEXTURE_COMPRESSION_HINT:
        HINT(texture_compression_hint);
    case GL_FRAGMENT_SHADER_DERIVATIVE_HINT:
        HINT(fragment_shader_derivative_hint);
        break;
...
{
    fprintf(stderr, "mglViewport called: ctx=%p, x=%d, y=%d, width=%d, height=%d\n", ctx, x, y, width, height);
    ...
    fprintf(stderr, "  Viewport set successfully on ctx=%p: [%u, %u, %u, %u]\n", ctx, ctx->state.viewport[0],
            ctx->state.viewport[1], ctx->state.viewport[2], ctx->state.viewport[3]);
...
GLboolean mglIsEnabled(GLMContext ctx, GLenum cap)
{
    ...
    case GL_BLEND:
        RET_CAP(blend);
    case GL_COLOR_LOGIC_OP:
        RET_CAP(color_logic_op);
    case GL_CULL_FACE:
        RET_CAP(cull_face);
    case GL_DEPTH_CLAMP:
        RET_CAP(depth_clamp);
    case GL_DEBUG_OUTPUT:
        RET_CAP(debug_output);
    case GL_DEBUG_OUTPUT_SYNCHRONOUS:
        RET_CAP(debug_output_synchronous);
    case GL_DEPTH_TEST:
        RET_CAP(depth_test);
    case GL_DITHER:
        RET_CAP(dither);
    case GL_FRAMEBUFFER_SRGB:
        RET_CAP(framebuffer_srgb);
    case GL_LINE_SMOOTH:
        RET_CAP(line_smooth);
    case GL_MULTISAMPLE:
        RET_CAP(multisample);
    case GL_POLYGON_SMOOTH:
        RET_CAP(polygon_smooth);
    case GL_POLYGON_OFFSET_FILL:
        RET_CAP(polygon_offset_fill);
    case GL_POLYGON_OFFSET_LINE:
        RET_CAP(polygon_offset_line);
    case GL_POLYGON_OFFSET_POINT:
        RET_CAP(polygon_offset_point);
    case GL_PROGRAM_POINT_SIZE:
        RET_CAP(program_point_size);
    case GL_PRIMITIVE_RESTART:
        RET_CAP(primitive_restart);
    case GL_SAMPLE_ALPHA_TO_COVERAGE:
        RET_CAP(sample_alpha_to_coverage);
    case GL_SAMPLE_ALPHA_TO_ONE:
        RET_CAP(sample_alpha_to_one);
    case GL_SAMPLE_COVERAGE:
        RET_CAP(sample_coverage);
    case GL_SAMPLE_MASK:
        RET_CAP(sample_mask);
    case GL_SCISSOR_TEST:
        RET_CAP(scissor_test);
    case GL_STENCIL_TEST:
        RET_CAP(stencil_test);
    case GL_TEXTURE_CUBE_MAP_SEAMLESS:
        RET_CAP(texture_cube_map_seamless);
...
{
    if (target >= GL_CLIP_DISTANCE0 && target >= GL_CLIP_DISTANCE7)
    {
        if (index >= 0 && index < ctx->state.var.max_clip_distances)
        {
