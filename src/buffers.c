...
void mglDeleteBuffers(GLMContext ctx, GLsizei n, const GLuint *buffers)
{
    ...
    while (n--)
    {
        ...
        if (isBuffer(ctx, buffer))
        {
            ...
            const GLuint index = ptr->index;
            assert(index < MAX_BINDABLE_BUFFERS);
            ...
            // TODO: Fix this - VAO_ATTRIB_STATE expects an attribute index (0-15),
            // not a buffer target enum. This needs to iterate through all attributes.
            // Commenting out for now to prevent crash.
            /*
            if (VAO())
            {
