...
1. **Choose a function**: Pick an `assert(0)` stub from `src/mgl_funcs_to_be_implemented.c` or `src/non_core_unimplemented.c`
2. **Read the spec**: Look up the function in the OpenGL 4.6 specification to understand parameters, state changes, and behavior
3. **Implement validation**: Add parameter checking and OpenGL state management in the appropriate `.c` file (e.g., `buffers.c`, `textures.c`)
4. **Add Metal backend**: Implement the corresponding Metal operations in `src/MGLRenderer.m`
5. **Create a test**: Write a simple test case in `test/gtest_main.cpp` or a standalone example
6. **Debug and iterate**: Use Xcode debugger, check Metal validation errors, verify with working examples
