...
static void surfaceHandleEnter(void *data, struct wl_surface *surface, struct wl_output *output)
{
    ...
    if (window->wl.monitorsCount + 1 > window->wl.monitorsSize)
    {
        ...
        window->wl.monitors = _glfw_realloc(window->wl.monitors, window->wl.monitorsSize * sizeof(_GLFWmonitor *));
    }
...
static void setIdleInhibitor(_GLFWwindow *window, GLFWbool enable)
{
    if (enable && !window->wl.idleInhibitor && _glfw.wl.idleInhibitManager)
    {
        ...
        if (!window->wl.idleInhibitor)
            _glfwInputError(GLFW_PLATFORM_ERROR, "Wayland: Idle inhibitor creation failed");
    }
...
static void setCursorImage(_GLFWwindow *window, _GLFWcursorWayland *cursorWayland)
{
    ...
    wl_pointer_set_cursor(_glfw.wl.pointer, _glfw.wl.pointerEnterSerial, surface, cursorWayland->xhot / scale,
    ...
    if (poll(fds, 3, timeout) > 0)
    {
        ...
        if (fds[1].revents & POLLIN)
        {
            ...
            if (read_ret == 8 && _glfw.wl.keyboardFocus)
            {
                ...
                {
                    _glfwInputKey(_glfw.wl.keyboardFocus, _glfw.wl.keyboardLastKey, _glfw.wl.keyboardLastScancode,
                                  GLFW_REPEAT, _glfw.wl.xkb.modifiers);
                }
            }
        }
...
int _glfwCreateWindowWayland(_GLFWwindow *window, const _GLFWwndconfig *wndconfig, const _GLFWctxconfig *ctxconfig,
...
void _glfwSetWindowMonitorWayland(_GLFWwindow *window, _GLFWmonitor *monitor, int xpos, int ypos, int width, int height,
                                  int refreshRate)
{
    if (monitor)
    {
