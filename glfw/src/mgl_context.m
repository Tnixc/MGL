...
static void makeContextCurrentMGL(_GLFWwindow *window)
{
    @autoreleasepool
    {
        ...
        else
        {
            ...
            MGLsetCurrentContext(createGLMContext(0, 0, 0, 0, 0, 0));
        }
...
GLFWbool _glfwInitMGL(void)
{
    ...
    {
        _glfwInputError(GLFW_API_UNAVAILABLE, "MGL: Failed to locate libmgl.dylib");
        return GLFW_FALSE;
    }
...
//
GLFWbool _glfwCreateContextMGL(_GLFWwindow *window, const _GLFWctxconfig *ctxconfig, const _GLFWfbconfig *fbconfig)
{
    if (ctxconfig->client == GLFW_OPENGL_ES_API)
    {
        _glfwInputError(GLFW_API_UNAVAILABLE, "MGL: OpenGL ES is not available on macOS");
        return GLFW_FALSE;
    }
    ...
    window->context.mgl.ctx =
        createGLMContext(GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV, GL_DEPTH_COMPONENT, GL_FLOAT, 0, 0);
    assert(window->context.mgl.ctx);

    if (window->context.mgl.ctx == nil)
    {
        _glfwInputError(GLFW_VERSION_UNAVAILABLE, "MGL: Failed to create MGL context");
        return GLFW_FALSE;
    }
...
GLFWAPI void *glfwGetMGLContext(GLFWwindow *handle)
{
    ...
    {
        _glfwInputError(GLFW_PLATFORM_UNAVAILABLE, "MGL: Platform not initialized");
        return nil;
    }
